...
class DragGameState extends GameState<DragGame> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onVerticalDragUpdate: (DragUpdateDetails details) {
        ...
        sendPosition(x, y);
        setState(() {
          player.transform.position.x = x.toDouble();
          player.transform.position.y = y.toDouble();
        });
        update();
      },
      child: Scaffold(
  ...
  GameObject player = new Player(0.0, 0.0);

  GameObject guest = new Player(100.0, 100.0);
